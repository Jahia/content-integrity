...
2.17 | 7.3.0.0 | <ul><li>Refactored the test results ID to embed the workspace name</li><li>Log the intermediate counts while counting the nodes to be scanned</li><li>[bugfix] Fixed the calculation of the number of nodes to scan when there's an error while iterating the JCR</li><li>[bugfix] Fixed the calculation of the fact that the external permission path is relative to the current site</li></ul>