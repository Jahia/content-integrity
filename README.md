...
2.8 | 7.3.0.0 | Implemented a sanity check on the ACE nodes, in order to detect ACE related to deleted users or groups<br>Implemented a factory method to create an empty errors list<br>[bugfix] Fixed the calculation of the number of nodes to scan when scanning the whole repository<br>[bugfix] Fixed the time calculation in case of error