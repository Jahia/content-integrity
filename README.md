...
2.16 | 7.3.0.0 | Implemented a check on the mandatory properties missing their value(s)<br>Implemented a check on the properties having a constaint to validate if the value match the constaint<br>Implemented a check to detect the properties set on the nodes but not being declared in any CND file<br>Sum up of the check durations at the end of the scan: sort the checks by duration